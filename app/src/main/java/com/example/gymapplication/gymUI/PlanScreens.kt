...
fun PlanScreen(viewModel: GymViewModel, navController: NavController) {
    ...
    if (showDiary) {
        ...
        BackHandler { selectedPlanId = null }
        PlanDetailScreen(
            plan = selectedPlan,
            viewModel = viewModel,
        ...
        var newPlanName by rememberSaveable { mutableStateOf("") }
        ...
        if (planToDelete != null) {
            AlertDialog(
                ...
                title = { Text("PLAN LÖSCHEN?", fontWeight = FontWeight.Black) },
                text = { Text("Möchtest du den Plan '${planToDelete.name}' wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.") },
                confirmButton = {
                    Button(
                        onClick = {
                            viewModel.deleteWorkoutPlan(planToDelete)
                            planToDeleteId = null
                        },
